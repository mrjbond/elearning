defmodule StationUpdatesTest.StationUpdateProducer do
  ...
  setup_all do
    ...
    assert :ok = Queue.bind(channel, queue, "station_updates", routing_key: "#")
  ...
  describe "#{__MODULE__}" do
    test "publish_event/2 produces a correctly configured event", %{
      ...
      station_id = Enum.random(~w(victoria waterloo westminster))

      event =
        StationUpdateEvent.new(
          {station_id, "Reduced escalator service.",
           [id: UUID.uuid4(), inserted_at: DateTime.utc_now()]}
        )

      assert {:ok, _seq_no} = StationUpdateProducer.publish_event(event)
